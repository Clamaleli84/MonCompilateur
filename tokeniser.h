// tokeniser.h : shared definition for tokeniser.l and compilateur.cpp

enum TOKEN {FEOF, UNKNOWN, NUMBER, ID, STRINGCONST, RBRACKET, LBRACKET, RPARENT, LPARENT, COMMA, 
SEMICOLON, COLON, DOT, ADDOP, MULOP, RELOP, NOT, ASSIGN, BEG, END, FOR, TO, WHILE, IF, THEN, ELSE, DO, DISPLAY, VAR, CHARTYPE, CHARCONST, CASE, OF, REPEAT, UNTIL, DOWNTO, HASHTAG};

enum TYPES {INTEGER, BOOLEAN, DOUBLE, CHAR, STRING};